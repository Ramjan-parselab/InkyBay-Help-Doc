...
export const loader = async({params}) => {
    ...
    const menuData = await prisma.footerMenu.findFirst({
        where: {
            id: parseInt(params?.id)
        },
        include:{
            footerMenuLanguage: true
        }
    });
...
export const action = async ({request}) => {
    ...
    if(target === "update-menu"){
        ...
        try{
            ...
            await prisma.footerMenu.update({
                data:{
                    name: name,
                    newTab: newTab,
                    url: url,
                    ...
                    if(item?.id){
                      return await prisma.footerMenuLanguage.update({
                            ...
                            data:{
                                ...
                                name: item?.lang == "en" ?  name : item.name,
                            }
                        })
                    }else{
                        return await prisma.footerMenuLanguage.create({
                            data:{
                                footerMenuId: parseInt(id),
                                lang: item.lang,
                                name: item.name,
                            }
                        })
            ...
            return {
                target: target,
                message: "Successfully ! menu has been updated",
                data: [],
            }
        }catch(error){
            console.log(error)
...
export default function Edit () {
    ...
    const handleTranslation = (event) => {
        ...
        let newValue = {};
        // Check is this language previously added to the categoryLanguage table 
        // if Yes then it formate newValue and if not then set newValue
        const previousData = formState.footerMenuLanguage.some((item) => item.lang === code);
        if(previousData){
            setFormState((prev)=> {
                const existing = prev.footerMenuLanguage.find((item) => item.lang === code);
                if(existing){
                    newValue = {
                        id: existing.id,
                        lang: existing.lang,
    ...
    const submitForm = async () => {
        ...
        if(validated) {
            submit({ target: "update-menu", data: JSON.stringify(formState) }, { method: "POST" });
        }
        else {
    ...
    useEffect(()=> {
        ...
        if(loaderData){
            ...
            if(loaderData?.data?.menu?.id) {
                const menuData = loaderData?.data?.menu;
                setFormState({
                    id: menuData?.id ? menuData?.id : "",
                    name: menuData?.name ? menuData?.name : "",
                    url: menuData?.url ? menuData?.url : "",
                    position: menuData?.position ? menuData?.position : "",
                    newTab: menuData?.newTab ?  menuData?.newTab : false,
                    status: menuData?.status ? menuData?.status : "",
                    footerMenuLanguage: menuData?.footerMenuLanguage ? menuData?.footerMenuLanguage : "",
                });
            }
        }
        setPageLoader(false)
    ...
    useEffect(() => {
        if (actionData) {
            if (actionData.target == "update-menu") {
                setButtonoader(false);
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <div className="my-5">
                                <div className="container mx-auto  py-4 px-6  bg-white text-gray-700">
                                    <div className="my-3">
                                        <h1 className="text-center text-2xl sm:text-3xl font-bold text-gray-700">Update footer menu</h1>
                                            ...
                                            <div className="relative flex flex-col">
                                                ...
                                                {languageData?.length > 0 && languageData?.map((language) => (
                                                    <div  className="my-2" key={language?.id}>
                                                        ...
                                                        {/* <input onChange={handleTranslation} value={language?.name ? formState?.categoryLanguage.filter((item)=> item.lang === language?.code)?.[0]?.name : ""} type="text" name={language?.code}  className="block w-full px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-gray-100 text-gray-900   outline-none" id={language?.code} /> */}
                                                        {formState?.footerMenuLanguage.some((item)=> item.lang === language?.code) ? (
                                                            <input onChange={handleTranslation} value={language?.name ? formState?.footerMenuLanguage.filter((item)=> item.lang === language?.code)?.[0]?.name : ""} type="text" name={language?.code}  className="block w-full px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-gray-100 text-gray-900   outline-none" id={language?.code} />
                                                        ) : (
                                                            <input onChange={handleTranslation} value={""} type="text" name={language?.code}  className="block w-full px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-gray-100 text-gray-900   outline-none" id={language?.code} />
                                                        )}
