...
export const loader = async ({ request })=> {
    ...
    if(query){
        menus = await prisma.footerMenu.findMany({
            select: {
...
export const action = async ({request}) => {
    ...
    if(target === "delete-menu"){
        const deleteId = formdata.get('data') || "";
        const menu = await prisma.footerMenu.findFirst({
            where: {
        ...
        if(menu?.id){
            try{
                await prisma.footerMenuLanguage.deleteMany({
                    where: {
                        footerMenuId: menu?.id
                    }
                });
                ...
                return {
                    target: target,
                    message: "Successfully ! menu has been deleted",
                    data: [],
                }
                ...
                return {
                    target: target,
                    message: "Menu delete failed please try again !!",
                    data: error,
                }
            }
        }
...
export default function Cotagory () {
    ...
    const deleteRow = (deleteId) => {
        if(deleteId){
            if(confirm("Do you want to delete this  ?? ")){
                submit({ target: "delete-menu", data: deleteId }, { method: "POST" });  
            }
        }
    }
    ...
    useEffect(()=> {
        setPageLoader(true)
        if(loaderData){
            if(loaderData?.data?.menus){
                setMenus(loaderData?.data?.menus)
            }
            // Get search data in params url by using "category" key
            if(newParams.get("q")){
                setSearchQuery(newParams.get("q"))
            }
        }
        setPageLoader(false)
    },[loaderData]);
            ...
            {pageLoader  ? (
                    ...
                    <div>
                        <div className="w-full flex justify-end items-end">
                            <Link to="/admin/footer/menus/create" className="inline-flex items-center gap-2 rounded bg-[#090808] px-8 py-3 text-sm font-semibold text-white transition-all mb-5">
                                Create new
                        ...
                        <div className="w-full flex justify-between items-center mb-3 mt-12 pl-3">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">Footer menu list</h3>
                            </div>
                            <div className="mx-3">
                                <div className="w-full max-w-sm min-w-[200px] relative">
                                <div className="relative">
                                    <input onChange={handleInputQuery} onKeyUp={getSearchQuery}  value={searchQuery} className="bg-white w-full pr-11 h-10 pl-3 py-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md"
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <table className="w-full text-left table-auto min-w-max">
                                ...
                                <tr className="border-b border-slate-300 bg-slate-50">
                                    ...
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">Position</th>
                                        ...
                                        menus?.map((menu)=> (
                                            <tr className="hover:bg-slate-50" key={menu?.id}>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="block font-semibold text-sm text-slate-800">{menu?.name}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{menu?.url}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{menu?.position}</p>
                                                </td>
                                                
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <div className="grid w-full grid-cols-2 gap-2">
                                                        <Link to={`/admin/footer/menus/${menu?.id}`} className="text-slate-500 hover:text-slate-700">
                                                        ...
                                                        <button onClick={()=> deleteRow(menu?.id) } className="text-slate-500 hover:text-slate-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
