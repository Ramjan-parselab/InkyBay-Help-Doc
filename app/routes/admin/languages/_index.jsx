...
export const loader = async ({ request })=> {
    ...
    if(language){
        languages = await prisma.language.findMany({
            select: {
                id: true, name:true, flug: true, code: true, status: true
            },
            where: {
    ...
    if(target === "delete-language"){
        ...
        if(language?.id){
            try{
                ...
                return {
                    target: target,
                    message: "Successfully ! Language has been deleted",
                    data: [],
                }
...
export default function Language () {
    ...
    const getSearchLanguage =  ()=> {
        newParams.set("language", searchLanguage);
    ...
    const deleteCategory = (deleteId) => {
        if(deleteId){
            if(confirm("Do you want to delete this language ?? ")){
                submit({ target: "delete-language", data: deleteId }, { method: "POST" });  
            }
        }
    }
    ...
    useEffect(()=> {
        setPageLoader(true)
        if(loaderData){
            if(loaderData?.data?.languages){
                setLanguages(loaderData?.data?.languages)
            }
            // Get search data in params url by using "category" key
            if(newParams.get("language")){
                setSearchLanguage(newParams.get("language"))
            }
        }
        setPageLoader(false)
    },[loaderData]);
            ...
            {pageLoader  ? (
                    ...
                    <div>
                        <div className="w-full flex justify-end items-end">
                            <Link to="/admin/languages/create" className="inline-flex items-center gap-2 rounded bg-[#090808] px-8 py-3 text-sm font-semibold text-white transition-all mb-5">
                                Create new
                        ...
                        <div className="w-full flex justify-between items-center mb-3 mt-12 pl-3">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">All Languages</h3>
                            </div>
                            <div className="mx-3">
                                <div className="w-full max-w-sm min-w-[200px] relative">
                                <div className="relative">
                                    <input onChange={handleInputCategory} onKeyUp={getSearchLanguage}  value={searchLanguage} className="bg-white w-full pr-11 h-10 pl-3 py-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md"
                                        placeholder="Search for language..."
                                    />
                                    <button onClick={getSearchLanguage}
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <table className="w-full text-left table-auto min-w-max">
                                ...
                                <tr className="border-b border-slate-300 bg-slate-50">
                                    ...
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">Flag</th>
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">code</th>
                                ...
                                <tbody>
                                    {languages?.length > 0 ? (
                                        languages?.map((language)=> (
                                            <tr className="hover:bg-slate-50" key={language?.id}>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="block font-semibold text-sm text-slate-800">{language?.name}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    ...
                                                    {/* <p className="text-sm text-slate-500">{language?.flug}</p> */}
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{language?.code}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{language?.status}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <div className="grid w-full grid-cols-2 gap-2">
                                                        <Link to={`/admin/languages/${language?.id}`} className="text-slate-500 hover:text-slate-700">
                                                        ...
                                                        <button onClick={()=> deleteCategory(language?.id) } className="text-slate-500 hover:text-slate-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
