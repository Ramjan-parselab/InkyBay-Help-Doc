...
export const action = async ({request}) => {
    ...
    if(target === "delete-doc"){
        const deleteId = formdata.get('data') || "";
        const helpDoc = await prisma.docs.findFirst({
            where: {
        ...
        if(helpDoc?.id){
            try{
                await prisma.docsLanguage.deleteMany({
                    where: {
                        docsId: helpDoc?.id
                    }
                });
...
export default function Cotagory () {
    ...
    const deleteRaw = (deleteId) => {
        if(deleteId){
            if(confirm("Do you want to delete this help doc ?? ")){
                submit({ target: "delete-doc", data: deleteId }, { method: "POST" });  
            }
        }
    }
    ...
    useEffect(()=> {
        setPageLoader(true);
        if(loaderData){
            if(loaderData?.data?.docs){
                setDocs(loaderData?.data?.docs)
            }
            ...
            {pageLoader  ? (
                    ...
                    <div>
                        <div className="w-full flex justify-end items-end">
                            <Link to="/admin/docs/create" className="inline-flex items-center gap-2 rounded bg-[#090808] px-8 py-3 text-sm font-semibold text-white transition-all mb-5">
                                Create new
                        ...
                        <div className="w-full flex justify-between items-center mb-3 mt-12 pl-3">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">All Documents</h3>
                            </div>
                            <div className="mx-3">
                                <div className="w-full max-w-sm min-w-[200px] relative">
                                <div className="relative">
                                    <input onChange={handleInputSearch} onKeyUp={getSearchDoc}  value={searchDoc} className="bg-white w-full pr-11 h-10 pl-3 py-2 bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md"
                                        placeholder="Search for doc..."
                                    />
                                    <button onClick={getSearchDoc}
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <table className="w-full text-left table-auto min-w-max">
                                ...
                                <tr className="border-b border-slate-300 bg-slate-50">
                                    ...
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">Total Reviews</th>
                                    ...
                                    {docs?.length > 0 ? (
                                        docs?.map((doc)=> (
                                            <tr className="hover:bg-slate-50" key={doc?.id}>
                                                ...
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{doc?.category?.name}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{doc?.helpDocReviews?.length}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                ...
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <div className="grid w-full grid-cols-2 gap-2">
                                                        <Link to={`/admin/docs/${doc?.id}`} className="text-slate-500 hover:text-slate-700">
                                                        ...
                                                        <button onClick={()=> deleteRaw(doc?.id) } className="text-slate-500 hover:text-slate-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
