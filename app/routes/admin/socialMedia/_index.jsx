    ...
    if(target === "delete-media"){
        ...
        if(socialMedia?.id){
            try{
                ...
                return {
                    target: target,
                    message: "Successfully ! Social media has been deleted",
                    data: [],
                }
                ...
                return {
                    target: target,
                    message: "Social media delete failed please try again !!",
                    data: error,
                }
            }
        }
...
export default function Language () {
    ...
    // Delete category 
    const deleteSIngleRow = (deleteId) => {
        if(deleteId){
            if(confirm("Do you want to delete this Media ?? ")){
                submit({ target: "delete-media", data: deleteId }, { method: "POST" });  
            }
        }
    }
    ...
    useEffect(()=> {
        setPageLoader(true)
        if(loaderData){
            if(loaderData?.data?.socials){
                SetSocialsData(loaderData?.data?.socials)
            }
            // Get search data in params url by using "category" key
            if(newParams.get("query")){
                setSearcQuery(newParams.get("query"))
            }
        }
        setPageLoader(false)
    },[loaderData]);
    ...
    // useEffect(()=> {
    //     if(actionData){
    //         if(actionData?.target == "delete-language"){
            ...
            {pageLoader  ? (
                    ...
                    <div>
                        <div className="w-full flex justify-end items-end">
                            <Link to="/admin/socials-media/create" className="inline-flex items-center gap-2 rounded bg-[#090808] px-8 py-3 text-sm font-semibold text-white transition-all mb-5">
                                Create new
                        ...
                        <div className="w-full flex justify-between items-center mb-3 mt-12 pl-3">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">All Social Media</h3>
                            </div>
                            <div className="mx-3">
                            ...
                            <table className="w-full text-left table-auto min-w-max">
                                ...
                                <tbody>
                                    {socialsData?.length > 0 ? (
                                        socialsData?.map((media)=> (
                                            <tr className="hover:bg-slate-50" key={media?.id}>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="block font-semibold text-sm text-slate-800">{media?.name}</p>
                                                </td>
                                               
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{media?.url}</p>
                                                </td>
                                                {/* <td className="p-4 border-b border-slate-200 py-5">
                                                ...
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <div className="grid w-full grid-cols-2 gap-2">
                                                        <Link to={`/admin/socials-media/${media?.id}`} className="text-slate-500 hover:text-slate-700">
                                                        ...
                                                        <button onClick={()=> deleteSIngleRow(media?.id) } className="text-slate-500 hover:text-slate-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
