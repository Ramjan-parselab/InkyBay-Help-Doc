...
export const loader = async({params}) => {
    ...
    let subCategoryInfo = [];
    if(category?.level === "LEVEL_3"){
        subCategoryInfo = await prisma.categories.findFirst({
            select: {
                id: true, parentCategory: true
            },
            where: {
                id: parseInt(category?.parentCategory),
                level: "LEVEL_2"
            },
        });
    }
...
export const action = async ({request}) => {
    ...
    if(target === "update-category"){
        ...
        const categoryInfo = await prisma.categories.findFirst({
        ...
        if(icon?.name){
            if(fs.existsSync(`public${categoryInfo?.icon}`)){
                fs.unlinkSync(`public${categoryInfo?.icon}`);
            }
        }
        ...
        try{
            ...
            await prisma.categories.update({
                data:{
                    name: name,
                    slug: slug,
                    icon: icon?.name ? uploadDir : categoryInfo?.icon,
                    // level: parentCategory ? "LEVEL_2" : "LEVEL_1" ,
                ...
                categoryLanguage.map(async (item) => {
                    ...
                    if(item?.id){
                      return await prisma.categoryLanguage.update({
                        ...
                        return await prisma.categoryLanguage.create({
                            data:{
            ...
            return {
                target: target,
                message: "Successfully ! category has been updated",
                data: [],
            }
        }catch(error){
            return {
...
export default function Edit () {
    ...
    const handleTranslation = (event) => {
        ...
        const previousData = formState.categoryLanguage.some((item) => item.lang === code);
        if(previousData){
            setFormState((prev)=> {
                const existing = prev.categoryLanguage.find((item) => item.lang === code);
                if(existing){
                    newValue = {
                        id: existing.id,
                        lang: existing.lang,
    ...
    const submitForm = async () => {
        ...
        if(validated) {
            ...
            formData.append("target", "update-category");
            formData.append("id", formState.id);
            formData.append("name", formState.name);
            formData.append("slug", formState.slug);
            formData.append("icon", formState.icon);
            formData.append("currentIcon", formState.currentIcon); 
    ...
    useEffect(()=> {
        ...
        if(loaderData){
            ...
            if(loaderData?.data?.category?.id) {
                const categoryData = loaderData?.data?.category;
                setFormState({
                    id: categoryData?.id ? categoryData?.id : "",
                    name: categoryData?.name ? categoryData?.name : "",
                    slug: categoryData?.slug ? categoryData?.slug : "",
                    icon: "",
                    currentIcon: categoryData?.icon ? categoryData?.icon : "",
                    level: categoryData?.level ? categoryData?.level : "",
                    serial: categoryData?.serial ? categoryData?.serial : "",
                    parentCategory: categoryData?.parentCategory,
                    status: categoryData?.status ? categoryData?.status : "",
                    categoryLanguage: categoryData?.categoryLanguage ? categoryData?.categoryLanguage : "",
                });
            }
        }
        setPageLoader(false)
    ...
    useEffect(() => {
        if (actionData) {
            if (actionData.target == "update-category") {
                setButtonoader(false);
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <div className="my-5">
                                <div className="container mx-auto  py-4 px-6  bg-white text-gray-700">
                                    <div className="my-3">
                                        <h1 className="text-center text-2xl sm:text-3xl font-bold text-gray-700">Update Category</h1>
                                            ...
                                            <div className="grid w-full grid-cols-2  gap-6 my-5">
                                                ...
                                                <div className="relative flex flex-col">
                                                    ...
                                                    <div className="my-2">
                                                        ...
                                                        <select onChange={handleParentCategory} value={formState?.parentCategory ? formState?.parentCategory : "" } className="block w-full px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-gray-100 text-gray-900   outline-none" id="parentCategory">
                                                    ...
                                                    <div className="my-2">
                                                            <label htmlFor="status" className="text-sm sm:text-md font-bold">Icon</label>
                                                            <div className="inline-flex h-12 w-12 items-end justify-end ml-4">
                                                                {formState?.currentIcon ? (
                                                                    <img src={formState?.currentIcon} alt={formState?.name} />
                                                                ) : (
