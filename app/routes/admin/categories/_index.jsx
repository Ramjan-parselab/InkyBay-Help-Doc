...
export const loader = async ({ request })=> {
    ...
    if(category){
        categories = await prisma.categories.findMany({
            select: {
                id: true, name:true, slug: true,  level: true, parentCategory: true, status: true,
                docs: {
                    select:{
                        id: true
                    }
    ...
    if(target === "delete-category"){
        ...
        if(category?.id){
            try{
                ...
                await prisma.categoryLanguage.deleteMany({
                    where: {
                ...
                return {
                    target: target,
                    message: "Successfully ! category has been deleted",
                    data: [],
                }
    ...
    const deleteCategory = (deleteId) => {
        if(deleteId){
            if(confirm("Do you want to delete this category ?? ")){
                submit({ target: "delete-category", data: deleteId }, { method: "POST" });  
            }
        }
    }
            ...
            {pageLoader  ? (
                    ...
                    <div>
                        <div className="w-full flex justify-end items-end">
                            <Link to="/admin/categories/create" className="inline-flex items-center gap-2 rounded bg-[#090808] px-8 py-3 text-sm font-semibold text-white transition-all mb-5">
                                Create new
                        ...
                        <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
                            <table className="w-full text-left table-auto min-w-max">
                                ...
                                <tr className="border-b border-slate-300 bg-slate-50">
                                    ...
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">Parent Category</th>
                                    <th className="p-4 text-sm font-normal leading-none text-slate-500">Total Docs</th>
                                        ...
                                        categories?.map((category)=> (
                                            <tr className="hover:bg-slate-50" key={category?.id}>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="block font-semibold text-sm text-slate-800">{category?.name}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{category?.slug}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{categories?.filter((item)=> item?.id === category?.parentCategory)?.[0]?.name}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{category?.docs?.length}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <p className="text-sm text-slate-500">{category?.status}</p>
                                                </td>
                                                <td className="p-4 border-b border-slate-200 py-5">
                                                    <div className="grid w-full grid-cols-2 gap-2">
                                                        <Link to={`/admin/categories/${category?.id}`} className="text-slate-500 hover:text-slate-700">
                                                        ...
                                                        <button onClick={()=> deleteCategory(category?.id) } className="text-slate-500 hover:text-slate-700">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6">
